...
if [ $2 ]; then
  ...
  NAMESPACE=`helm ls | grep $1 | awk '{print $(NF)}'`
fi
...
if [ $NAMESPACE ]; then
    ...
    kubectl delete all -l release=$1 -n $NAMESPACE --grace-period=0 --force
	...
	do
	    ...
	    kubectl delete $type $resource --namespace=$NAMESPACE --grace-period=0 --force
	done
    done
